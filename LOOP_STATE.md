...
- 2026-07-18 — Issue #18 (pass 2): switched distribution model per human review — `main` no longer carries the committed ncc bundles for `build`/`notify-slack` (removed `.github/actions/{build,notify-slack}/dist`, dropped the `.gitignore` un-ignore so the blanket `dist/` rule covers them again); added `.github/workflows/release.yml`, which builds the bundles with `npm run build:actions` on a `vX.Y.Z` tag push (or manual dispatch), commits them onto that tag, force-moves the rolling major alias (`vX`), and cuts/updates a GitHub Release; `action.yml` files untouched (still `node20` / `dist/index.js`, now arriving via the release tag instead of `main`) — docs/examples updated from `@main` to `@v1` accordingly — branch `claude/issue-18-implementer-verifier-xsxokd` — implementer→verifier: **VERDICT APPROVE** (+ idempotency guard added to `release.yml` re-runs); committed & pushed — [needs-human] (release-tag distribution is a public-facing consumer contract change; human to cut the first `v1.0.0` tag)
