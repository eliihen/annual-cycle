...
- 2026-07-31 — `.gitignore` ignored `.worktrees/` but the Agent tool's `isolation: 'worktree'` actually creates worktrees under `.claude/worktrees/`, so every worktree-isolated implementer run left untracked files behind and tripped the Stop hook's git-check — [auto-fixable] — fixed inline this run (see Done), shipped in PR #43.
